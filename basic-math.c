uint64 xorRandom(uint64 *s) {
	// A shift-register generator has a reproducible behavior across platforms.
	return *s ^= *s << 13, *s ^= *s >> 7, *s ^= *s << 17 ;
}

uint64 xorRandint(uint64 *seed, uint64 min, uint64 max) {
	// Return a random number within the specified range.
	return min + xorRandom(seed) % (max - min + 1);
}

int isTinyPrime(const uint32 n) {
...
// the implementation is designed for beginners, so there aren't many preprocessing "tricks".
	...
	return (uint32)(res % mod);
#endif
}

uint32 powerModulo(uint64 n, uint64 exp, const uint32 mod) {
	// Return (n ^ exp) % mod
	uint32 res = 1;
	for (n %= mod; exp; exp & 1 ? res = multiplicationModulo(res, n, mod) : 0, n = multiplicationModulo(n, n, mod), exp >>= 1);
	return res;
}

int kroneckerSymbol(uint32 a, uint32 b) {
...
uint32 tonelliShanks(const uint32 n, const uint32 mod) {
	...
	const uint32 a = (uint32) (n % mod);
	uint32 res = kroneckerSymbol(a, mod) == 1, b, c, d, e, f, g, h;
	if (res)
		switch (mod & 7) {
			case 3 :
			case 7 :
				res = powerModulo(a, (mod + 1) >> 2, mod);
				break;
			case 5 :
				res = powerModulo(a, (mod + 3) >> 3, mod);
				if (multiplicationModulo(res, res, mod) != a) {
					b = powerModulo(2, (mod - 1) >> 2, mod);
					res = multiplicationModulo(res, b, mod);
				}
				break;
			default :
				if (a == 1)
					...
					for (c = mod - 1, d = 2; d < mod && powerModulo(d, c >> 1, mod) != c; ++d);
					for (e = 0; !(c & 1); ++e, c >>= 1);
					f = powerModulo(a, c, mod);
					b = powerModulo(d, c, mod);
					for (h = 0, g = 0; h < e; h++) {
						d = powerModulo(b, g, mod);
						d = multiplicationModulo(d, f, mod);
						d = powerModulo(d, 1 << (e - h - 1), mod);
						if (d == mod - 1)
							g += 1 << h;
					}
					f = powerModulo(a, (c + 1) >> 1, mod);
					b = powerModulo(b, g >> 1, mod);
					res = multiplicationModulo(f, b, mod);
				}
		}
	return res;
}

uint32 modularInverse(uint32 ra, uint32 rb) {
