...
uint32 multiplication_modulo(uint64 a, uint64 b, const uint32 mod) {
...
uint32 power_modulo(uint64 n, uint64 exp, const uint32 mod) {
	// Return (n ^ exp) % mod
	uint32 res = 1;
...
int kronecker_symbol(uint32 a, uint32 b) {
...
uint32 tonelli_shanks(const uint32 n, const uint32 mod) {
	...
	uint32 res = kronecker_symbol(a, mod) == 1, b, c, d, e, f, g, h;
	if (res)
		switch (mod & 7) {
			case 3 :
			case 7 :
...
uint32 modular_inverse(uint32 ra, uint32 rb) {
